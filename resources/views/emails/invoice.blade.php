...
<table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="border-collapse: collapse; margin-bottom: 30px; border-radius: 8px; overflow: hidden;">
    ...
    <tr>
        ...
        <td style="padding: 12px 16px; border: 1px solid #E5E7EB;">{{ $invoice->issue_date ? $invoice->issue_date->format('F d, Y') : '—' }}</td>
    </tr>
    <tr>
        <td style="padding: 12px 16px; background-color: #F9FAFB; border: 1px solid #E5E7EB; font-weight: 600;">Due Date</td>
        <td style="padding: 12px 16px; border: 1px solid #E5E7EB;">{{ $invoice->due_date ? $invoice->due_date->format('F d, Y') : '—' }}</td>
    </tr>
    <tr>
...
<p style="text-align: center; margin: 30px 0;">
    <a href="{{ $checkoutUrl }}" style="background-color: #2563EB; color: #ffffff; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: 600;">
        💳 Pay Invoice Now
    </a>
</p>
